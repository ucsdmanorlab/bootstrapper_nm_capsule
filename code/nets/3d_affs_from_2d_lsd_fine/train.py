...
def train(
    ...
    pipeline += gp.DeformAugment(
        control_point_spacing=voxel_size * gp.Coordinate(2, 10, 10),
        jitter_sigma=gp.Coordinate(2, 2, 2) * voxel_size[1],
        spatial_dims=3,
        subsample=4,
        scale_interval=(0.5, 1.5)
    )
    pipeline += gp.ShiftAugment(prob_slip=0.2, prob_shift=0.2, sigma=3, p=0.8)
    ...
    pipeline += GammaAugment(input_lsds, slab=(1, 1, -1, -1), p=0.2)
    pipeline += ImpulseNoiseAugment(input_lsds, pixel_p=0.05, p=0.2)

    # smooth the batch by different sigmas to simulate noisy predictions
    pipeline += SmoothAugment(input_lsds, p=0.2)
