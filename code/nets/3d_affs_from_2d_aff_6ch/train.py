...
def create_pipeline(voxel_size, net_config):
    ...
    in_neighborhood = [[0, *x] for x in in_neighborhood]  # add z-dim, pipeline is 3D
    ...
    pipeline += gp.AddAffinities(
        ...
        affinities=input_lsds,          # keep the key name 'input_lsds' to avoid a rename cascade
        dtype=np.float32,
    )
...
def train(
    ...
    model = LitModel(input_shape=[net_config['inputs']['2d_affs']['dims'],] + net_config['input_shape'])
    ...
    checkpoints = glob.glob('model_*')
    latest_checkpoint = natsorted(checkpoints)[-1] if checkpoints else None
