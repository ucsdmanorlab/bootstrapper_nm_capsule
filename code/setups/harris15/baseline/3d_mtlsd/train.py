...
def labelled_locations(labels_path, mask_path=None, max_points=4000, seed=42, input_shape=None):
    ...
    Points are at voxel centers so a single-section output ROI lands exactly on
    the annotated section. We center on label objects (labels>0) restricted to
    the valid mask: for per-voxel sparse the mask already equals labels>0, while
    for whole-section masks this picks the actual objects inside the section
    instead of sampling background, so every crop carries positive labels.
    """
    ...
    # Keep only labelled voxels whose centered 3D request fits strictly inside the
    # array, so gp.SpecifiedLocation never centers a request on the boundary -- that
    # trips a gunpowder Pad edge case ("index can't contain negative values") that
    # crashes the 3d train at a random iteration. Relax for thin volumes (e.g. liconn,
    # whose 3D input is deeper than the slab) where no interior remains.
    if input_shape is not None:
        shp = np.array(fg.shape)
        margin = np.array(input_shape) // 2 + 4
        interior = np.all((idx >= margin) & (idx < shp - margin), axis=1)
        if interior.sum() >= 50:
            idx = idx[interior]
...
def train(
    ...
    # get lsd task params
    sigma_vox = net_config["outputs"]["3d_lsds"]["sigma"]
    # sigma is specified in VOXELS of physical context. Use an ISOTROPIC world-unit
    # scalar = sigma_vox * the finest (xy) voxel: ~sigma_vox voxels of context in xy
    # and proportionally FEWER in z (anisotropy-aware), so it fits the thin 4-section
    # 3d output instead of exceeding it. (A per-axis 10-voxel sigma made the z context
    # 10 sections -> degenerate LSDs, which stalled epi.) Matches the working tree's
    # ~10-xy-voxel / ~2-z-voxel LSD scale (sigma 80 nm at 8 nm voxels).
    sigma = sigma_vox * voxel_size[-1]
    lsd_downsample = net_config["outputs"]["3d_lsds"]["downsample"]
    # lsd downsample>1 needs every axis's context, floor(3*sigma/voxel), to be a
    # multiple of the downsample factor, or AddLocalShapeDescriptor asserts ("not a
    # multiple of downsampling factor"). Trim sigma by a few nm so it aligns: a
    # sub-voxel change to the descriptor scale (e.g. epi 750 -> 704 nm at ds 2).
    while lsd_downsample > 1 and sigma > 1 and not all(
        (3 * sigma) // v % lsd_downsample == 0 for v in voxel_size
    ):
        sigma -= 1
    ...
    for sample in samples:
        ...
        src += (
            ...
            + gp.SpecifiedLocation(
                labelled_locations(sample["labels"], mask_path, input_shape=input_shape),
    ...
    pipeline += DefectAugment(
        raw, prob_missing=0.1 if section_augment else 0.0, prob_low_contrast=0.1, prob_deform=0.0
    )
    ...
    pipeline += gp.BalanceLabels(gt_affs, affs_weights, mask=gt_affs_mask, slab=(3, -1, -1, -1))
    ...
    pipeline += gp.Squeeze(
        [raw, gt_lsds, pred_lsds, lsds_weights, gt_affs, pred_affs, affs_weights]
    )
