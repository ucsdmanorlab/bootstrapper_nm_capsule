...
class BatchRenorm(nn.Module):
    """Batch Renormalization (Ioffe 2017), per channel, for (N, C, *spatial).
    Trains at batch_size=1 and uses global running stats at eval, so tiled
    prediction has no per-tile normalization seams (arXiv:2503.19545)."""
    def __init__(self, num_features, dims, eps=1e-3, momentum=0.01,
                 rmax=3.0, dmax=5.0, warmup=3000, ramp=1000):
        super().__init__()
        self.dims = dims
        self.eps, self.momentum = eps, momentum
        self.rmax, self.dmax, self.warmup, self.ramp = rmax, dmax, warmup, ramp
    ...
    def _v(self, s):
        return s.view(1, -1, *([1] * self.dims))

    def _bounds(self):
        t = float(self.steps.item())
        if t < self.warmup:
            return 1.0, 0.0
        f = min(1.0, (t - self.warmup) / max(1, self.ramp))
        return 1.0 + f * (self.rmax - 1.0), f * self.dmax

    def forward(self, x):
        if self.training:
            red = [0] + list(range(2, x.dim()))
            bmean = x.mean(red)
            bstd = x.std(red, unbiased=False) + self.eps
            rmax, dmax = self._bounds()
            r = (bstd.detach() / self.running_std).clamp(1.0 / rmax, rmax)
            d = ((bmean.detach() - self.running_mean) / self.running_std).clamp(-dmax, dmax)
            x = (x - self._v(bmean)) / self._v(bstd) * self._v(r) + self._v(d)
            self.running_mean += self.momentum * (bmean.detach() - self.running_mean)
            self.running_std += self.momentum * (bstd.detach() - self.running_std)
            self.steps += 1
        else:
            x = (x - self._v(self.running_mean)) / self._v(self.running_std)
        return x * self._v(self.weight) + self._v(self.bias)


def _norm_layer(norm, channels, dims):
    if not norm or norm == "none":
        return None
    if norm == "batchrenorm":
        return BatchRenorm(channels, dims)
    if norm == "batchnorm":
        return {2: nn.BatchNorm2d, 3: nn.BatchNorm3d}[dims](channels)
    if norm == "instancenorm":
        return {2: nn.InstanceNorm2d, 3: nn.InstanceNorm3d}[dims](channels, affine=True)
...
class ConvPass(torch.nn.Module):
    ...
    def __init__(
        ...
        for index, kernel_size in enumerate(kernel_sizes):
            ...
            nl = _norm_layer(norm, out_channels, self.dims)
            if nl is not None:
                layers.append(nl)
                ...
                nn.ModuleList(
                    [
                        Upsample(
                            ...
                            mode="bilinear" if constant_upsample else "transposed_conv",
