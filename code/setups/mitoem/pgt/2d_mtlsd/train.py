...
def labelled_locations(labels_path, mask_path=None, max_points=5000, seed=42, bg_ratio=0.3):
...
def train(
    ...
    # 2D->3D net (world-unit sigma would vary wildly per voxel size: 80 nm is ~10
    ...
    for sample in samples:
        ...
        src += (
            ...
            + gp.SpecifiedLocation(
                labelled_locations(sample["labels"], mask_path, bg_ratio=0.3),
    ...
    pipeline += gp.NoiseAugment(raw, p=0.05)
    ...
    pipeline += SmoothAugment(raw, p=0.1)
    pipeline += DefectAugment(raw, prob_missing=0.05 if (adj_slices > 1 and section_augment) else 0.0, prob_low_contrast=0.05)
